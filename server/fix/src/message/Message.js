...
import {
  ...
  Symbol,
  TimeInForce,
} from './../constants/ConstantsField.js';
...
export const calculateBodyLength = (value) => {
  const startLength = value.indexOf(TAG_MSGTYPE) === -1 ? 0 : value.indexOf(TAG_MSGTYPE),
    endLength = value.lastIndexOf(TAG_CHECKSUM) === -1 ? value.length : value.lastIndexOf(TAG_CHECKSUM);
...
export const validateMessage = (message) => {
  ...
  result = result.sort((a, b) => {
    const sort = parseInt(a.position, 10) < parseInt(b.position, 10) ? -1 : 1;
    return sort;
  });
...
export default class Message {
  ...
  getBriefDescription() {
    ...
    if (this.getField(LeavesQty)) {
      ...
      returnValue = nonEmpty`${quantity} @${
        lastPrice || String(lastPrice) === '0' ? lastPrice.toFixed(2) : null
      } ${this.getField(LeavesQty).name.replace('LeavesQty', 'LvsQty')} ${leavesQuantity}`;
      ...
      if (this.getField(Price)) {
        ...
        returnValue = nonEmpty`${side} ${orderQuantity} ${symbol ? symbol.toUpperCase() : null} ${
  ...
  validateBodyLength(value) {
    const startLength = this.string.indexOf(TAG_MSGTYPE) === -1 ? 0 : this.string.indexOf(TAG_MSGTYPE),
      endLength =
        this.string.lastIndexOf(TAG_CHECKSUM) === -1
          ? this.string.length
          : this.string.lastIndexOf(TAG_CHECKSUM),
